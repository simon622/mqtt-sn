...
public class SessionHandler extends MqttsnConsoleAjaxRealmHandler {
    ...
    protected SessionBean populateBean(String clientId) throws MqttsnException {
        ...
        Optional<IClientIdentifierContext> context = registry.getSessionRegistry().lookupClientIdSession(clientId);
        if(context.isPresent()){
            ISession session = registry.getSessionRegistry().getSession(context.get(), false);
            if(session != null){
                ...
                bean.protocolVersion = session.getProtocolVersion() + "";
                ...
                Set<ISubscription> subs = registry.getSubscriptionRegistry().readSubscriptions(session);
                bean.subscriptions = subs.stream().map(s -> new SubscriptionBean(s)).collect(Collectors.toList());

                Set<ITopicRegistration> regs = registry.getTopicRegistry().getRegistrations(session);
    ...
    class SubscriptionBean {
        ...
        public SubscriptionBean(ISubscription s){
    ...
    class RegistrationBean {
        ...
        public RegistrationBean(ITopicRegistration s){
            topicFilter = s.getTopicPath().toString();
            alias = s.getAliasId();
