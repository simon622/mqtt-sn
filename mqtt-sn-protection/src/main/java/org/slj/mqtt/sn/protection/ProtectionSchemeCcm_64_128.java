...
{
    private static final Logger logger = LoggerFactory.getLogger(ProtectionSchemeCcm_64_128.class);
	...
	public ProtectionSchemeCcm_64_128(String name, byte index)
	{
		this.name=name;
		this.index=index;
		nominalTagLengthInBytes=8;
		nominalTagLengthInBits=(short)(nominalTagLengthInBytes*8);
		blockSizeInBytes=16;
	...
	public byte[] unprotect(byte[] associatedData, byte[] encryptedPayload, byte[] tagToBeVerified, byte[] key) throws MqttsnSecurityException
	{
        ...
        byte[] encryptedPayloadToBeDecrypted = new byte[encryptedPayload.length+tagToBeVerified.length];
